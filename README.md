...
本 Demo 用 **三个角色、三个页面** 演示完整闭环：**创建/启用规则 → 开发者提案模板 → 管理员审核上架**。
...
| 角色 | 页面 | 能做什么 |
| --- | --- | --- |
| 🦞 个人虾用户 | 我的任务 | 「我的任务」开关列表（含 启用 / 待审核 / 已驳回 状态）、**3 步向导** 自己创建规则（选触发器 → 写动作内容 → 命名）、从模板市场一键添加、查看每条规则的「主动触发执行」历史 |
| 🛠️ 开发者 | 模板策划 | 提交「触发器 × 默认动作」模板提案（简化表单：名称 / 触发器 / 动作 / 简介），看到自己所有模板的状态 |
| ✅ 平台管理员 | 所有任务审核 | **审核所有任务**：待审核队列同时包含「用户自建规则」与「开发者模板提案」，各带「用户规则 / 模板提案」标签，**1 步通过 / 驳回** 即可（驳回填理由） |
